...
def isDirExists(dir_path):
	...
	'''
	if User_bucket.objects(na = dir_path).count() > 0: # 根据目录路径进行查询
		return True
	else :
		return False
...
def isFileExists(filename, dirId):
	...
	'若存在，则返回True，不存在，则返回False
	'''
	if User_bucket.objects(Q(na = filename) & Q(did = dirId)).count() > 0: # 根据文件名以及文件所在路径进行查询
		return True
	else :
		return False
...
def getDirId(parent_path):
	...
	'返回父目录的id
	'''
	pr = User_bucket.objects(na = parent_path)
	if pr.count() > 0:
		p = pr[0]
		return p.id
	else:
