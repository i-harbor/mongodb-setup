	...
	if recursive_flag == 1:
		if isDirExists(path) == False:  # 若目录在数据库中不存在
			with switch_collection(Mybucket, collection_name) as Mybucket:
				rootpath = Mybucket(na = path, fod = False) # 创建对象
				rootpath.save() # 添加到db
	...
	for file in files:
		...
		if os.path.isdir(current_path): # 若是目录
			if not isDirExists(current_path): # 若该目录在db中不存在
				...
				with switch_collection(Mybucket, collection_name) as Mybucket:
					p = Mybucket(na = current_path, fod = False, did = parentId) # 创建对象
					p.save() # 添加到db
...
def process_one_file(filepath, dir_path, filename, cover_mode):
	...
	if isFileExists(filename, dirId) : # 若文件在db已存在
		if cover_mode :
			with switch_collection(Mybucket, collection_name) as Mybucket:
				for u in Mybucket.objects(Q(na = filename) & Q(did = dirId)): # 删除原记录
					u.delete()
				size = os.path.getsize(filepath) # 获取文件大小，单位字节
				f = Mybucket(na = filename, fod = True, did = dirId, si = size) # 添加新纪录
				f.save()
	else :
		size = os.path.getsize(filepath) # 获取文件大小，单位字节
		with switch_collection(Mybucket, collection_name) as Mybucket:
...
def main():
	...
	'参数：数据库名，ip，端口
	'''
	connect("test20", host = '10.0.86.201', port=27017)

	# 设置待扫描的目录路径
	dir_path = 'D:/'

	if dir_path != '/': #若目录不是linux的根目录'/'
		# 去掉目录路径后的'/'符号
